...
```  
 ...
 The table shows all classes have an even proportion of NA values in the observations. We can begin building our models using the theory that NA's relate to lack of sensor output and therefore a directional output of zero we won't bias any of the classes unfairly. We can then further test our models by imputing via knn imputation preprocessing if nescessary. 
...
mod.rf <- train(classe ~ ., data = train,
                   ...
                   trControl = ctrl)
```  
```{r predict_xgb, warnings = FALSE, message=FALSE}
...
The deafult settings for caret uses three different values for mtry,
2,12 and 23. A good value for mtry is the rounded down squareroot of the number of predictors. This would be an mtry of
  
```{r}
ctrl <- trainControl(method = "cv",
                     number = 10,
                     verboseIter= FALSE,
                     rfgrid <- expand.grid(mtry= c(4,6,7,8,10,12)))
##train model
...
confusionMatrix(pred.rf.tun, test$classe)
```
